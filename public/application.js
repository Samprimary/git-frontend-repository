...
var createRaider = function createRaider(event) {
  event.preventDefault();
  var data = getFormFields(event);
  // console.log('data at createRaider is ', data)
  api.createSubmit(data).then(ui.createSuccess).catch(ui.createError);
  $(event.target).trigger('reset');
};

var indexRaiders = function indexRaiders(event) {
  event.preventDefault();
  api.indexSubmit().then(ui.indexSuccess).catch(ui.indexError);
};

var getRaider = function getRaider(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.getSubmit(data);
};

var updateRaiderList = function updateRaiderList(event) {
  $('.update-raider-list').show();
}; // may not work in time

var updateRaider = function updateRaider(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  var raiderId = $(event.target).closest('ul').attr('data-id');
  api.updateSubmit(data, raiderId).then(ui.updateSuccess).catch(ui.updateError);
  $(event.target).trigger('reset');
};

var deleteRaider = function deleteRaider(event) {
  event.preventDefault();
  var raiderId = $(event.target).closest('ul').attr('data-id');
  api.deleteSubmit(raiderId).then(ui.deleteSuccess).catch(ui.deleteError);
};
...
var onGetRaiders = function onGetRaiders(event) {
  event.preventDefault();
  api.getRaiders().then(ui.getRaidersSuccess).catch(ui.failure);
};

var onClearRaiders = function onClearRaiders(event) {
  event.preventDefault();
  ui.clearRaiders();
};
...
var indexSubmit = function indexSubmit() {
  return $.ajax({
    method: 'GET',
...
var getSubmit = function getSubmit(data) {
  return $.ajax({
    method: 'GET',
...
var createSubmit = function createSubmit(data) {
  console.log('data is', data);
  return $.ajax({
    method: 'POST',
    url: config.apiUrl + '/raiders/',
...
var updateSubmit = function updateSubmit(data, raiderId) {
  return $.ajax({
    method: 'PATCH',
...
var deleteSubmit = function deleteSubmit(raiderId) {
  return $.ajax({
    url: config.apiUrl + '/raiders/' + raiderId,
    method: 'DELETE',
    headers: {
...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://desolate-wildwood-48917.herokuapp.com/',
...
var signOutAnnouncer = function signOutAnnouncer() {
...
var indexSuccess = function indexSuccess(data) {
  ...
  var showRaidersHtml = handlebars({ raiders: data.raiders });
  $('#raiders-content').html('');
  $('#raiders-content').append(showRaidersHtml);
};

var indexError = function indexError(error) {
  $('#announcer').html('We could not get the raiders.', error);
};

var deleteSuccess = function deleteSuccess(raiderId) {
  $('#announcer').html('That raider is history.');
  $('#content').html('');
};
var deleteError = function deleteError(error) {
...
var findError = function findError(error) {
  $('#announcer').html('That raider evaded detection.', error);
};

var updateSuccess = function updateSuccess(raiderId) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  "
    ...
    + "</h4></li>\n      <li>\n        <form class=\"update-raider\">\n          <input name=\"raider[name]\" type=\"text\" placeholder=\"Name\">\n          <br>\n          <input name=\"raider[power]\" type=\"integer\" placeholder=\"Power Level\">\n          <br><br>\n          <button type=\"submit\" class=\"btn btn-primary\">Update raider!</button>\n        </form>\n      </li>\n      <li><button class=\"delete-raider btn btn-danger\">Delete raider</button></li>\n    </ul>\n</div>\n\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "\n        Power Level: "
    + container.escapeExpression(container.lambda(((stack1 = blockParams[1][0]) != null ? stack1.power : stack1), depth0))
    + "\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "        No powerlevel found.\n      ";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.raiders : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
